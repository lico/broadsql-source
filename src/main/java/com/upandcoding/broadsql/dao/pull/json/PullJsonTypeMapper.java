...
/**
 * Validates that a query column's JDBC type is one {@code PULL ... AS JSON} can write, before any byte
 * of the target file is written - see docs/PULL_TO_TEXT.md, "Type handling". Deliberately narrow, the
 * same "ordinary business data" set every other PULL destination accepts - duplicated independently here
 * too, not a call into any other type mapper (every PULL destination kind shares the command's grammar
 * and nothing else).
 *
 * <p>Large objects ({@code BLOB}/{@code CLOB}/{@code NCLOB}), binary data, and structural/vendor-specific
 * types are rejected explicitly rather than being stringified into a JSON value of unclear shape.
 */
final class PullJsonTypeMapper {

	private PullJsonTypeMapper() {
	}
		...
		switch (jdbcType) {
			...
			default:
				throw new BroadSQLException("Column '" + columnLabel + "' has type " + typeName + " (JDBC type " + jdbcType
						+ "), which PULL ... AS JSON cannot write - large objects (BLOB/CLOB/NCLOB), binary, and "
