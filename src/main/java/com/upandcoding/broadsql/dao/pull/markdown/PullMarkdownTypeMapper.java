...
/**
 * Validates that a query column's JDBC type is one {@code PULL ... AS MD} can write, before any byte of
 ...
 */
final class PullMarkdownTypeMapper {

	private PullMarkdownTypeMapper() {
	}
		...
		switch (jdbcType) {
			...
			default:
				throw new BroadSQLException("Column '" + columnLabel + "' has type " + typeName + " (JDBC type " + jdbcType
						+ "), which PULL ... AS MD cannot write - large objects (BLOB/CLOB/NCLOB), binary, and "
