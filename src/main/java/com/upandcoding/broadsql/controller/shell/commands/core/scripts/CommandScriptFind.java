...
/**
 * Full-text, case-insensitive content search across the scripts catalog: {@code SCRIPT FIND <term>}.
 * Same shape as {@code LIB FIND} (see its Javadoc), applied to the {@code Scripts} catalog: always
 * searches the whole catalog regardless of instance or environment.
 */
public class CommandScriptFind extends Command {

	public CommandScriptFind() {
		super("SCRIPT FIND", "SC FI", "SCFI");
	}
	...
	public void execute(String query) throws BroadSQLException {
		...
		if (rows.isEmpty()) {
			console.println("No script matches '" + term + "'");
			return;
		}

		console.println("Scripts catalog entries matching '" + term + "' (" + rows.size() + "):");
	...
	public String getDescription() {
		return ("Full-text, case-insensitive search across the whole scripts catalog (content, not just file names), regardless of instance or environment");
	}
